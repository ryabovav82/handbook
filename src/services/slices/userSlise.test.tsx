...
describe('проверки для userSlice', () => {
  it('проверка getUser', () => {
    ...
    expect(stateWithData.data).toEqual({
  ...
  it('проверка loginUser pending и fulfilled', () => {
  ...
  it('проверка loginUser  rejected', async () => {
    const initialState = {
      isAuthChecked: false,
    ...
    try {
      await store.dispatch(
        login.rejected(mockError, '', { email: '', password: '' })
      );
    } catch (error) {}
